...
\makeatletter
\input{mccp_chain_metab_arrows.tex}
...
\newcommand{\ChemostatSnapOutflowTrunkXwNoteNode}[2]{%
  ...
  \makeatother
  \pgfmathsetmacro{\csoutflownodeabove}{int(\csoutflownodecy>\mccpchainpipey)}%
  \makeatletter
...
\newcommand{\ChemostatSnapOutflowTrunkDraw}{%
  ...
  \ifChemostatSnapOutflowTrunkXwSet
    ...
    \draw[csoutflowline, gray!65!black, line cap=butt]
      (\csoutflowtrunkxwdraw,\mccpchainpipey)
      -- (\csoutflowmergex,\mccpchainpipey);%
  ...
  \draw[cspipe, gray!65!black, shorten >=\ChemostatSnapRefPipeShorten]
    (\csoutflowmergex,\mccpchainpipey) -- (csOutlet);%
}
...
\newcommand{\ChemostatSnapInitStackY}[1]{%
  ...
  \pgfmathsetmacro{\csinflowstacky}{\mccpchainpipey + (\csinitstackcenter-\csinitstackindex)*\csinitstackstep}%
}
...
\newcommand{\ChemostatSnapRestackMone}[1]{\mccpchainrestackmet{#1}{csmballMone}}
\newcommand{\ChemostatSnapRestackMtwo}[1]{\mccpchainrestackmet{#1}{csmballMtwo}}
\newcommand{\ChemostatSnapRestackWaste}[1]{\mccpchainrestackmet{#1}{csmballWaste}}
\newcommand{\ChemostatSnapRestackShared}[1]{\mccpchainrestackmet{#1}{csmballShared}}
% Gradient metabolism arrow through organism center (steps 2--4).
\newcommand{\ChemostatSnapMetabThroughOrg}[6]{%
  \mccpchaindrawmetabarrow{#1}{#2}{#3}{#4}{#5}{#6}%
}
% Task~A when $M_2$ pool is not collinear with $M_1$ (center exits toward sink).
\newcommand{\ChemostatSnapMetabThroughOrgCtrSink}[6]{%
  \mccpchaindrawmetabarrowCtrSink{#1}{#2}{#3}{#4}{#5}{#6}%
}
% Curved lane above/below organism (step~5 energy panel; optional motifs).
\newcommand{\ChemostatSnapMetabLane}[6]{%
  \mccpchaindrawmetabarrowLane{#1}{#2}{#3}{#4}{#5}{#6}{u}%
}
...
\newcommand{\ChemostatSnapTank}{%
  ...
  \coordinate (csInlet) at (\mccpchainrimL,\mccpchainpipey);%
  \coordinate (csOutlet) at (\mccpchainrimR,\mccpchainpipey);%
  ...
  \foreach \t in {0.28, 0.58, 0.86} {%
    ...
    \ifdim\csarrowtip pt>\dimexpr#2 pt\relax
      ...
      \draw[-{Stealth[scale=\ChemostatSnapTrunkFlowArrowScale]}, #3, line width=\ChemostatSnapRefMetabLW]
        (\csarrowx,\mccpchainpipey) -- ++(\csarrowlen,0);%
    \else
      \draw[-{Stealth[scale=\ChemostatSnapTrunkFlowArrowScale]}, #3, line width=\ChemostatSnapRefMetabLW]
        (\csarrowx,\mccpchainpipey) -- ++(0.20,0);%
    \fi
  }%
}
...
\newcommand{\ChemostatSnapInitMerge}[3]{%
  ...
  \draw[#1, shorten <=\ChemostatSnapRefMetabShorten]
    (#2.east)
    .. controls (\csinitmergecxone,#3) and (\csinitmergecxtwo,\mccpchainpipey) ..
    (\csinitmergex,\mccpchainpipey);%
}

\newcommand{\ChemostatSnapInitBranch}[5]{%
  \pgfmathsetmacro{\csinflowpooly}{#4}%
  \pgfmathsetmacro{\csinflowabove}{int(\csinflowpooly>\mccpchainpipey)}%
  \makeatletter
  ...
  \ifnum\csinflowabove=1
    \draw[#1, shorten <=\ChemostatSnapRefMetabShorten]
      ...
      -- (\csinflowedgex,\mccpchainpipey+\ChemostatSnapInitBranchBend)
      .. controls (\csinflowedgex,\mccpchainpipey)
                  and (\csinitjunction,\mccpchainpipey) ..
      (\csinitjunction,\mccpchainpipey);%
  \else
    \draw[#1, shorten <=\ChemostatSnapRefMetabShorten]
      (#2.north)
      -- (\csinflowedgex,\csinflowedgey)
...
\newcommand{\ChemostatSnapInflowMerge}[3]{%
  ...
  \draw[#1, shorten <=\ChemostatSnapRefMetabShorten]
    (#2.east)
    .. controls (\csinflowmergecxone,#3) and (\csinflowmergecxtwo,\mccpchainpipey) ..
    (\csinflowmergex,\mccpchainpipey);%
}
...
\newcommand{\ChemostatSnapInflowBranch}[5]{%
  ...
  \ifnum\csinflowabove=1
    \draw[#1, shorten <=\ChemostatSnapRefMetabShorten]
      (#2.south)
      -- (\csinflowedgex,\csinflowedgey)
      -- (\csinflowedgex,\mccpchainpipey+\ChemostatSnapInflowBranchBend)
      .. controls (\csinflowedgex,\mccpchainpipey)
                  and (\csinflowjunction,\mccpchainpipey) ..
      (\csinflowjunction,\mccpchainpipey);%
  \else
    \draw[#1, shorten <=\ChemostatSnapRefMetabShorten]
      (#2.north)
      -- (\csinflowedgex,\csinflowedgey)
...
\newcommand{\ChemostatSnapOutflowBranch}[2]{%
  ...
  \makeatother
  \pgfmathsetmacro{\csoutflowabove}{int(\csoutflowpooly>\mccpchainpipey)}%
  \makeatletter
  ...
  \pgfmathsetmacro{\csoutflowvroom}{abs(\csoutflowedgey-\mccpchainpipey)}%
  ...
  \ifnum\csoutflowabove=1
    \draw[csoutflowline, #1]
      (#2.south)
      -- (\csoutflowedgex,\mccpchainpipey+\csoutflowbend)
      .. controls (\csoutflowedgex,\mccpchainpipey)
                  and (\csoutflowteex,\mccpchainpipey) ..
      (\csoutflowjunction,\mccpchainpipey);%
  \else
    \draw[csoutflowline, #1]
      (#2.north)
      -- (\csoutflowedgex,\mccpchainpipey-\csoutflowbend)
...
\newcommand{\ChemostatSnapOutflowMerge}[3]{%
  ...
  \pgfmathsetmacro{\csoutflowmergecyone}{%
    \csoutflowmergeendy+\ChemostatSnapOutflowMergeStackPull*(\mccpchainpipey-\csoutflowmergeendy)}%
  \draw[csoutflowline, #1, shorten >=\ChemostatSnapRefMetabShorten]
    (\csoutflowmergex,\mccpchainpipey)
    .. controls (\csoutflowmergecxone,\mccpchainpipey) and (\csoutflowmergeendx,\csoutflowmergecyone) ..
    (#2.west);%
}
...
\newcommand{\ChemostatSnapEnergyPanelDraw}{%
    ...
    \mccpchaindrawmetabarrowLane{metabEnA}{csEnM1}{csEnOrg}{csEnM2out}{blue!65!black}{yellow!55!black}{u}%
    \mccpchaindrawmetabarrowLane{metabEnB}{csEnM2in}{csEnOrg}{csEnW}{yellow!55!black}{gray!65!black}{l}%
    \mccpchainrestackmet{csEnM1}{csmballMone}%
    \mccpchainrestackmet{csEnM2out}{csmballMtwo}%
    \mccpchainrestackmet{csEnM2in}{csmballMtwo}%
    \mccpchainrestackmet{csEnW}{csmballWaste}%
...
\newcommand{\ChemostatSnapOutflowPopB}{%
  ...
  \pgfmathsetmacro{\csoutflowystackMtwo}{\mccpchainpipey+2*\csoutflowstackdy}%
  \pgfmathsetmacro{\csoutflowystackWb}{\mccpchainpipey+\csoutflowstackdy}%
  \pgfmathsetmacro{\csoutflowystackMone}{\mccpchainpipey}%
  \pgfmathsetmacro{\csoutflowystackOrg}{\mccpchainpipey-\csoutflowstackdy}%
  \pgfmathsetmacro{\csoutflowystackWc}{\mccpchainpipey-2*\csoutflowstackdy}%
