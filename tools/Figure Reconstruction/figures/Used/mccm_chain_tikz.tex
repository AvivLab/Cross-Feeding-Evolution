...
\def\mccmchaindrawmetab@genchunk#1#2#3#4#5{%
  \ifnum#5=1
    \foreach \j in {0,1,...,\mccmchaingradlast} {%
      \mccmchainmetab@segloc{\j}{1}{}%
      \pgfmathsetmacro{\mccmchainuA}{#3+\mccmchainlocA*(#4-#3)}%
      \pgfmathsetmacro{\mccmchainuB}{#3+\mccmchainlocB*(#4-#3)}%
      \coordinate (mbMetPtA) at ($(#1)!{\mccmchainlocA}!(#2)$);%
      \coordinate (mbMetPtB) at ($(#1)!{\mccmchainlocB}!(#2)$);%
      \pgfmathsetmacro{\mccmchainmixA}{100*(1-2*\mccmchainuA)}%
      \pgfmathsetmacro{\mccmchainmixB}{100*(1-2*(\mccmchainuB-0.5))}%
      \ifdim\mccmchainuA pt<0.5pt
        \pgfmathsetmacro{\mccmchainmixval}{int(round(\mccmchainmixA))}%
        \edef\mccmchainmixexp{\mccmchainmixval}%
        \def\mccmchaincolA{mccmchaincA}%
        \def\mccmchaincolB{mccmchaincB}%
      \else
        \pgfmathsetmacro{\mccmchainmixval}{int(round(\mccmchainmixB))}%
        \edef\mccmchainmixexp{\mccmchainmixval}%
        \def\mccmchaincolA{mccmchaincB}%
        \def\mccmchaincolB{mccmchaincC}%
      \fi
      \ifnum\j=\mccmchaingradlast
        \def\mccmchaincapline{mccmchainmetabline@butt}%
      \else
      ...
      \fi
      \draw[\mccmchaincapline,
        draw=\mccmchaincolA!\mccmchainmixexp!\mccmchaincolB] (mbMetPtA) -- (mbMetPtB);%
    }%
    \pgfmathsetmacro{\mccmchainuTip}{#3+\mccmchainarrowshaftstop*(#4-#3)}%
    \ifdim\mccmchainuTip pt<0.5pt
      \pgfmathsetmacro{\mccmchainmixTip}{int(round(100*(1-2*\mccmchainuTip)))}%
      \edef\mccmchainmixTipExp{\mccmchainmixTip}%
      \def\mccmchaintipColA{mccmchaincA}%
      \def\mccmchaintipColB{mccmchaincB}%
    \else
      \pgfmathsetmacro{\mccmchainmixTip}{int(round(100*(1-2*(\mccmchainuTip-0.5))))}%
      \edef\mccmchainmixTipExp{\mccmchainmixTip}%
      \def\mccmchaintipColA{mccmchaincB}%
      \def\mccmchaintipColB{mccmchaincC}%
    \fi
    \coordinate (mbMetShaftEnd) at ($(#1)!\mccmchainarrowshaftstop!(#2)$);%
    \edef\mccmchaintipcolor{\mccmchaintipColA!\mccmchainmixTipExp!\mccmchaintipColB}%
    \mccmchaindrawmetab@shaftbridge{#1}{#2}{\mccmchaintipcolor}%
    \mccmchaindrawmetab@splithead{#1}{mbMetShaftEnd}{#2}{\mccmchaintipcolor};%
  \else
    \foreach \j in {0,1,...,\mccmchaingradlast} {%
      \mccmchainmetab@segloc{\j}{0}{}%
      \pgfmathsetmacro{\mccmchainuA}{#3+\mccmchainlocA*(#4-#3)}%
...
\def\mccmchaindrawmetabarrowGeneralist#1#2#3#4#5#6#7#8{%
  \colorlet{mccmchaincA}{#6}%
  \colorlet{mccmchaincB}{#7}%
  \colorlet{mccmchaincC}{#8}%
  \mccmchainboundpoint{#2}{#3}{0}{square}{mbGenMone}%
  \mccmchainboundpoint{#3}{#2}{0}{circle}{mbGenOrgIn}%
  \coordinate (mbGenOrgC) at (#3.center);%
  \mccmchainboundpoint{#3}{#4}{0}{circle}{mbGenOrgOut}%
  \mccmchainboundpoint{#4}{#3}{0}{square}{mbGenMtwoNear}%
  \mccmchainboundpoint{#4}{#5}{0}{square}{mbGenMtwoFar}%
  \mccmchainboundpoint{#3}{#4}{0}{circle}{mbGenOrgReIn}%
  \mccmchainboundpoint{#3}{#5}{0}{circle}{mbGenOrgReOut}%
  \mccmchainboundpoint{#5}{#3}{0}{square}{mbGenWaste}%
  \draw[mccmchainmetabline, shorten <=0.6pt,
    draw=mccmchaincA] (mbGenMone) -- (mbGenOrgIn);%
  \mccmchaindrawmetab@genchunk{mbGenOrgIn}{mbGenOrgC}{0.125}{0.25}{0}%
  \mccmchaindrawmetab@genchunk{mbGenOrgC}{mbGenOrgOut}{0.25}{0.375}{0}%
  \mccmchaindrawmetab@genchunk{mbGenOrgOut}{mbGenMtwoNear}{0.375}{0.5}{0}%
  \draw[mccmchainmetabline, draw=mccmchaincB] (mbGenMtwoNear) -- (mbGenMtwoFar);%
  \mccmchaindrawmetab@genchunk{mbGenMtwoFar}{mbGenOrgReIn}{0.5}{0.625}{0}%
  \mccmchaindrawmetab@genchunk{mbGenOrgReIn}{mbGenOrgC}{0.625}{0.75}{0}%
  \mccmchaindrawmetab@genchunk{mbGenOrgC}{mbGenOrgReOut}{0.75}{0.875}{0}%
  \mccmchaindrawmetab@genchunk{mbGenOrgReOut}{mbGenWaste}{0.875}{1}{1}%
}
...
\newcommand{\mccmchainchemostatpanel}[2]{%
  \begin{scope}[shift={(0,#1)}]
    ...
    \node[demolabel, anchor=north] at (dupDemoFit.south |- 0,\mccmchaindemolabely) {Duplication};
    \node[demolabel, anchor=north] at (dieO.south |- 0,\mccmchaindemolabely) {Death};
    \node[demolabel, anchor=north] at (mutDemoFit.south |- 0,\mccmchaindemolabely) {Mutation};
