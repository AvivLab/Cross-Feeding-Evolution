#!/usr/bin/env bash
# Render figures/Used/mccp_four_conditions_tikz.tex -> figures/Used/mccp_four_conditions.pdf
set -euo pipefail
...
BUILD_DIR="$FIG_DIR/.build_mccp_four_conditions"
STEM="mccp_four_conditions"
WRAPPER="$FIG_DIR/mccp_four_conditions_standalone.tex"
...
if command -v "$PDFCROP" >/dev/null 2>&1; then
  tmp="$(mktemp -t mccp_four_conditions_crop.XXXXXX.pdf)"
