#!/usr/bin/env bash
# Render simulation_loop_combined_grid.tex -> figures/Used/simulation_loop_combined.pdf
set -euo pipefail
...
BUILD_DIR="$FIG_DIR/.build_simulation_loop_combined"
STEM="simulation_loop_combined"
WRAPPER="$FIG_DIR/simulation_loop_combined_standalone.tex"
...
# Chemostat snapshot PDFs are embedded in the flowchart.
"$SCRIPT_DIR/build_chemostat_snapshot_pdfs.sh"

mkdir -p "$BUILD_DIR"
latexmk -pdf -interaction=nonstopmode -f -cd \
...
if command -v "$PDFCROP" >/dev/null 2>&1; then
  tmp="$(mktemp -t simulation_loop_combined_crop.XXXXXX.pdf)"
